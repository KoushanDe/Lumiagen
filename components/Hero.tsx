...
const Hero: React.FC = () => {
  ...
  // Increased base opacity from 0.25 to 0.5 for more intensity
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLElement>) => {
    if (containerRef.current) {
      ...
      // Calculate Continuous Color Shift based on X position
      // Left (0%) = Teal (45, 212, 191)
      // Right (100%) = Blue (59, 130, 246)
      const width = rect.width;
      const factor = Math.min(Math.max(x / width, 0), 1); // Clamp between 0 and 1
      
      const r = Math.round(45 + factor * (59 - 45));
      const g = Math.round(212 + factor * (130 - 212));
      const b = Math.round(191 + factor * (246 - 191));
      
      // Increased alpha to 0.5 for "More Powerful" feel
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-void">
        ...
        {/* Layer 0: Volumetric Fog (Rotating Gradients) */}
        ...
        {/* Layer 3: Shooting Stars (Data Beams) */}
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        {/* Interactive Mouse Glow (Soft Bloom - Matches Grid Color) */}
        <div 
        ...
        {/* Dynamic Side Glow on Hover - Significantly Reduced & Matching Flashlight Color */}
        <div 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full text-center">
        ...
        <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight text-white mb-6 leading-[1.1] group cursor-default animate-enter [animation-delay:150ms] relative">
          ...
          <span className="inline-block pb-2 transition-all duration-500 ease-out hover:scale-[1.02]">
            {/* 5-Stop Continuous Gradient: Green -> White -> Blue -> White -> Green */}
